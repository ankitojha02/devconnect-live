...
export default function FeedPage() {
  ...
  const searchUsers = async (value: string) => {
    try {
      ...
      const res = await fetch(`${API}/user/search/users?search=${value}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  const followUser = async (id: string) => {
    try {
      ...
      if (isFollowing(id)) {
        updatedFollowing = updatedFollowing.filter((f: string) => f !== id);
      } else {
      ...
      <nav className="sticky top-0 z-50 border-b border-zinc-800 bg-black/80 backdrop-blur-xl">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-4 lg:px-8">
          ...
          <div className="flex items-start gap-3 sm:items-center sm:gap-4">
            ...
            <h1 className="hidden text-3xl font-black sm:block">DevConnect</h1>
          </div>
                ...
                <div className="max-h-[420px] space-y-3 overflow-y-auto">
                  {users.map((dev) => (
                    ...
                    >
                      ...
                      <div
                        onClick={() => router.push(`/user/${dev._id}`)}
                      ...
                      >
                        {isFollowing(dev._id) ? "Following" : "Follow"}
                      </button>
                    </div>
                  ))}
                </div>
...
>
  <Bell className="h-6 w-6" />
            ...
            <div className="h-12 w-12 overflow-hidden rounded-2xl bg-yellow-400">
              <img
                src={user?.avatar || "/developers.png"}
                alt="profile"
                className="h-full w-full object-cover"
              />
            </div>
                ...
                {users.map((dev) => (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h3 className="text-sm font-bold">{dev.name}</h3>

                        <p className="text-xs text-zinc-500">@{dev.username}</p>
                      </div>
                    </div>
        ...
        <aside className="order-3 hidden space-y-8 lg:block">
          ...
          <div className="rounded-[28px] border border-zinc-800 bg-[#111111] p-4 sm:p-5 lg:p-6">
            <div className="mb-6 flex items-center gap-3">
              ...
              <h2 className="text-2xl font-black">Find Developers</h2>
            </div>
          ...
          <div className="rounded-[28px] border border-zinc-800 bg-[#111111] p-4 sm:p-5 lg:p-6">
            <div className="mb-6 flex items-center gap-3">
              ...
              <h2 className="text-2xl font-black">Trending Communities</h2>
            </div>
                ...
                followersUsers.map((dev) => (
                  ...
                  >
                    ...
                    <div>
                      ...
                      <p className="text-sm text-zinc-500">@{dev.username}</p>
